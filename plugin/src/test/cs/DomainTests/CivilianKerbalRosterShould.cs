...
namespace CivilianPopulation.DomainTests
{
    ...
    public class CivilianKerbalRosterShould
    {
		...
		{
			CivilianKerbalRoster roster = new CivilianKerbalRoster();
			roster.add(new CivilianKerbal("Jeb", "Pilot", true, -1));
			Assert.AreEqual(
				"{\"Jeb\":{\"expectingBirthAt\":-1, \"name\":\"Jeb\", \"male\":true, \"trait\":\"Pilot\"}}",
				roster.toString()
			);
		}
		...
		public void decode_non_empty_roster()
		{
			CivilianKerbalRoster roster = new CivilianKerbalRoster();
			roster.add(new CivilianKerbal("Jeb", "Pilot", true, -1));
			roster.add(new CivilianKerbal("Val", "Pilot", false, 123456));
        ...
        public void list_2_members()
        {
            CivilianKerbalRoster roster = new CivilianKerbalRoster();
            roster.add(new CivilianKerbal("Jeb", "Pilot", true, -1));
            roster.add(new CivilianKerbal("Val", "Pilot", false, -1));
            int count = 0;
            foreach (CivilianKerbal kerbal in roster.list())
            {
                count++;
